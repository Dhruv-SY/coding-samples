...
* Robustness: exclude the Benin-Niger (BEN-NER) border pair
****************************************************

do "code/00_setup.do"

* Remove Niger border pair and re-encode bp to avoid ghost FE levels
drop if inlist(country_pair, "BEN-NER")
drop bp
...
*---------------------------------------------------*
file open tex_iv using "results/results_noniger_iv.tex", write replace

file write tex_iv "\begin{table}[htbp]" _n
file write tex_iv "\centering" _n
file write tex_iv "\caption{IV-2SLS Effect of Child Marriage Across Bandwidths (Excluding Niger Border Pair)}" _n
file write tex_iv "\label{tab:iv_bw_noniger}" _n
...
*---------------------------------------------------*
file open tex_rf using "results/results_noniger_reducedform.tex", write replace

file write tex_rf "\begin{table}[htbp]" _n
file write tex_rf "\centering" _n
file write tex_rf "\caption{Reduced-Form Effect of Treatment Across Bandwidths (Excluding Niger Border Pair)}" _n
file write tex_rf "\label{tab:rf_bw_noniger}" _n

...
* Robustness: exclude women who migrated around the time of marriage
****************************************************

do "code/00_setup.do"

* Exclude women who migrated in the same window as their marriage
capture drop age_moved
gen age_moved = v012 - v104 if !missing(v104)
capture drop years_between_move_marriage
gen years_between_move_marriage = v511 - age_moved if !missing(age_moved, v511)
drop if years_between_move_marriage >= 0 & years_between_move_marriage <= 3
...
*---------------------------------------------------*
file open tex_iv using "results/results_migration_iv.tex", write replace

file write tex_iv "\begin{table}[htbp]" _n
file write tex_iv "\centering" _n
file write tex_iv "\caption{IV-2SLS Effect of Child Marriage Across Bandwidths (Excluding Recent Migrants)}" _n
file write tex_iv "\label{tab:iv_bw_migration}" _n
...
file write tex_iv "\hline" _n
file write tex_iv "\multicolumn{4}{p{0.9\linewidth}}{\footnotesize Notes: Women who migrated within three years of their marriage are excluded. For each outcome and bandwidth, the table reports the IV-2SLS coefficient on child marriage with clustered standard errors in parentheses and the number of observations used in the IV regression. All regressions include border-pair fixed effects, border-pair-specific slopes and kinks, geographic controls, ethnicity fixed effects, survey-year fixed effects, and individual controls. Stars indicate significance: * \$p<0.10\$, ** \$p<0.05\$, *** \$p<0.01\$.}" _n
...
*---------------------------------------------------*
file open tex_rf using "results/results_migration_reducedform.tex", write replace

file write tex_rf "\begin{table}[htbp]" _n
file write tex_rf "\centering" _n
file write tex_rf "\caption{Reduced-Form Effect of Treatment Across Bandwidths (Excluding Recent Migrants)}" _n
file write tex_rf "\label{tab:rf_bw_migration}" _n
...
file write tex_rf "\hline" _n
file write tex_rf "\multicolumn{4}{p{0.9\linewidth}}{\footnotesize Notes: Women who migrated within three years of their marriage are excluded. For each outcome and bandwidth, the table reports the reduced-form coefficient on treated with clustered standard errors in parentheses and the number of observations used in the regression. All regressions include border-pair fixed effects, border-pair-specific slopes and kinks, geographic controls, ethnicity fixed effects, survey-year fixed effects, and individual controls. Stars indicate significance: * \$p<0.10\$, ** \$p<0.05\$, *** \$p<0.01\$.}" _n

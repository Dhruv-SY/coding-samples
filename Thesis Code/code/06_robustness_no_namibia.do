...
* Robustness: exclude all border pairs involving Namibia
****************************************************

do "code/00_setup.do"

* Remove Namibia border pairs and re-encode bp to avoid ghost FE levels
drop if inlist(country_pair, ///
    "Angola-Namibia", ///
    "Namibia-SouthAfrica", ///
    "Namibia-Zambia", ///
    "Namibia-Zimbabwe")
...
*---------------------------------------------------*
file open tex_iv using "results/results_nonamibia_iv.tex", write replace

file write tex_iv "\begin{table}[htbp]" _n
file write tex_iv "\centering" _n
file write tex_iv "\caption{IV-2SLS Effect of Child Marriage Across Bandwidths (Excluding Namibia Border Pairs)}" _n
file write tex_iv "\label{tab:iv_bw_nonamibia}" _n
...
*---------------------------------------------------*
file open tex_rf using "results/results_nonamibia_reducedform.tex", write replace

file write tex_rf "\begin{table}[htbp]" _n
file write tex_rf "\centering" _n
file write tex_rf "\caption{Reduced-Form Effect of Treatment Across Bandwidths (Excluding Namibia Border Pairs)}" _n
file write tex_rf "\label{tab:rf_bw_nonamibia}" _n

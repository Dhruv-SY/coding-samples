...
* IV-2SLS and reduced-form estimates split by urban/rural residence
****************************************************

do "code/00_setup.do"

* v025 excluded from controls since sample is split by urban/rural status
global X "c.v012"
...
* Save original microdata so each urban/rural loop starts fresh
tempfile masterdata
save `masterdata', replace

*---------------------------------------------------*
* 6) LOOP OVER RESIDENCE STATUS
*---------------------------------------------------*
foreach r in 1 2 {

    use `masterdata', clear

    local rname = cond(`r'==1, "urban", "rural")
    di "========================================"
    di "Residence group: `rname'"
    di "========================================"
    ...
    *---------------------------------------------------*
    file open tex_iv using "results/results_`rname'_iv.tex", write replace

    file write tex_iv "\begin{table}[htbp]" _n
    file write tex_iv "\centering" _n
    file write tex_iv "\caption{IV-2SLS Effect of Child Marriage Across Bandwidths (`rname')}" _n
    file write tex_iv "\label{tab:iv_bw_`rname'}" _n
    ...
    *---------------------------------------------------*
    file open tex_rf using "results/results_`rname'_reducedform.tex", write replace

    file write tex_rf "\begin{table}[htbp]" _n
    file write tex_rf "\centering" _n
    file write tex_rf "\caption{Reduced-Form Effect of Treatment Across Bandwidths (`rname')}" _n
    file write tex_rf "\label{tab:rf_bw_`rname'}" _n
